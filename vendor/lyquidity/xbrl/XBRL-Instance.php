...
class XBRL_Instance
{
	...
	public static function FromInstanceDocumentWithExtensionTaxonomy( $instance_document, $compiledLocation, $className = 'XBRL', $useCache = false )
	{
		try
		{
			...
			if ( ! $instance->initialise( $instance_document, $compiledLocation, $useCache ) )
			{
				return false;
			}
		}
	...
	private function initialise( $instance_document, $taxonomy_file = null, $useCache = false )
	{
		...
		// $this->instance_xml = new SimpleXMLElement( file_get_contents( $instance_document ) );
            ...
            $data = file_get_contents($instance_document);




            if(!$this->instance_xml = simplexml_load_string($data, "SimpleXMLElement", LIBXML_DTDLOAD)) {

                $this->instance_xml = simplexml_load_string(html_entity_decode($data,ENT_COMPAT, "UTF-8"), "SimpleXMLElement", LIBXML_DTDLOAD);

            }

	 

       // $this->instance_xml = new SimpleXMLElement(  $data  );
		...
		if ( ! is_a( $this->instance_xml, 'SimpleXMLElement' ) )
		{
