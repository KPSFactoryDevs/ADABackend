...
use lyquidity\xml\MS\XmlNamespaceManager;
 ...
 /**
  * Implements the filter class for the PeriodEnd filter
  * http://www.xbrl.org/Specification/periodFilters/REC-2009-06-22/periodFilters-REC-2009-06-22.html#term-period-end-filter
  */
class PeriodEnd extends PeriodDateTime
{
	/**
	...
	public function process( $localName, $taxonomy, $roleUri, $linkbaseHref, $label, $node, $domNode, $log )
	{
		...
		{
			$log->formula_validation( "period end filter", "Missing mandatory date attribute",
				array(
					'label' => $this->label,
					'error' => 'xbrlpfe:missingDateAttribute'
				)
			);
		}
		...
		$this->time = property_exists( $attributes, "time" )
			? $attributes->time
			: null;
	...
	public function toQuery( $variableSet, $factVariableBinding )
	{
		return $this->time
				 ?	"(xfi:period(.)[\n" .
					...
					"  then (xfi:period-end(.) eq fn:dateTime({$this->date}, {$this->time})) \n" .
					"  else fn:false() \n" .
					"])"
				:  "(xfi:period(.)[\n" .
					"  if (xfi:is-start-end-period(.)) \n" .
					"  then (xfi:period-end(.) eq fn:dateTime({$this->date}, xs:time('24:00:00'))) \n" .
					"  else fn:false() \n" .
					"])";
	}
