...
use lyquidity\xml\MS\XmlNamespaceManager;
  ...
  * CAN BE OPTIMIZED
  * (see ModelFormulaObject.py ModelEntityRegexpIdentifier class)
  */
class RegexpIdentifier extends Filter
{
	/**
	...
	public function process( $localName, $taxonomy, $roleUri, $linkbaseHref, $label, $node, $domNode, $log )
	{
		...
		{
			$log->formula_validation( "Entity filter", "No 'pattern' attribute in the RegexpIdentifier element", array(
				'roleuri' => $roleUri,
				'label' => $label,
				'localname' => $localName,
			) );
	...
	{
		return "fn:matches(xfi:fact-identifier-value(.),'{$this->pattern}')";
	}
