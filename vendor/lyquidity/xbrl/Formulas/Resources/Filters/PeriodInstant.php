...
use lyquidity\xml\MS\XmlNamespaceManager;
  ...
  */
class PeriodInstant extends PeriodDateTime
{
	/**
	...
	public function toQuery( $variableSet, $factVariableBinding )
	{
		return $this->time
				 ?	"(xfi:period(.)[\n" .
					...
					"  then (xfi:period-instant(.) eq fn:dateTime({$this->date}, {$this->time})) \n" .
					"  else fn:false() \n" .
					"])"
				:  "(xfi:period(.)[\n" .
					"  if (xfi:is-instant-period(.)) \n" .
					"  then (xfi:period-instant(.) eq fn:dateTime({$this->date}, xs:time('24:00:00'))) \n" .
					"  else fn:false() \n" .
					"])";

		// return "(xfi:period(.)[\n" .
		// 		"  if (xfi:is-instant-period(.)) \n" .
		// 		"  then (xfi:period-instant(.) eq (fn:dateTime('{$this->date}', '{$time}') $day ) ) \n" .
		// 		"  else fn:false() \n" .
		// 		"])";
	}
}
