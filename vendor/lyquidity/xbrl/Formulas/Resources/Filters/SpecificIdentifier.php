...
use lyquidity\xml\MS\XmlNamespaceManager;
  ...
  * http://www.xbrl.org/specification/entityfilters/rec-2009-06-22/entityfilters-rec-2009-06-22.html#sec-specific-entity-identifier-filter
  * CAN BE OPTIMIZED
  * (see ModelFormulaObject.py ModelEntitySpecificIdentifier class)
  */
class SpecificIdentifier extends Filter
{
	/**
	...
	/**
	 * A compiled version of $value
	 * @var XPath2Expression $valueXPath2Expression
	 */
	public $valueXPath2Expression = null;
	...
	public function process( $localName, $taxonomy, $roleUri, $linkbaseHref, $label, $node, $domNode, $log )
	{
		...
		{
			$log->formula_validation( "Entity filter", "No 'scheme' attribute in the SpecificIdentifier element", array(
				'roleuri' => $roleUri,
				'label' => $label,
				'localname' => $localName,
			) );
		...
		if ( ! property_exists( $attributes, 'value' ) && \XBRL::isValidating() )
		{
			$log->formula_validation( "Entity filter", "No 'value' attribute in the SpecificIdentifier element", array(
				'roleuri' => $roleUri,
				'label' => $label,
				'localname' => $localName,
			) );
		...
		{
			$this->value = trim( $attributes->value );
		}
	...
	{
		return "(xfi:fact-identifier-value(.) eq {$this->value} and xfi:fact-identifier-scheme(.) eq {$this->scheme})";
	}
	...
	public function validate( $variableSet, $nsMgr )
	{
		...
		{
			\XBRL_Log::getInstance()->formula_validation( "Specific identifier filter", "Failed to compile scheme expression",
				array(
		...
		try
		{
			if ( $this->value )
			{
				if ( ! $this->valueXPath2Expression )
				{
					$this->valueXPath2Expression = XPath2Expression::Compile( $this->value, $nsMgr );
				}
			}
		}
		catch ( \Exception $ex )
