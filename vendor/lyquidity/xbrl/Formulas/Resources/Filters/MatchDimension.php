...
use lyquidity\xml\MS\XmlNamespaceManager;
  ...
  */
class MatchDimension extends MatchFilter
{
	/**
	...
	public function process( $localName, $taxonomy, $roleUri, $linkbaseHref, $label, $node, $domNode, $log )
	{
		...
		if ( ! property_exists( $attributes, 'dimension' ) )
		{
			...
			{
				$log->formula_validation( "Match filter", "No 'dimension' attribute in the $localName element", array(
					'roleuri' => $roleUri,
					'label' => $label,
					'localname' => $localName,
				) );
		...
		{
			$this->dimension = qname( trim( $attributes->dimension ), $node->getDocNamespaces( true ) );
