...
/**
  ...
  */
class GeneralMeasures extends Filter
{
	/**
	...
	public function process( $localName, $taxonomy, $roleUri, $linkbaseHref, $label, $node, $domNode, $log )
	{
		...
		{
			$log->formula_validation( "Unit Filters", "Missing GeneralMeasures filter GeneralMeasures 'test' attribute", array(
	...
	{
		return "(xfi:is-numeric(fn:node-name(.)) and xfi:unit(.)[{$this->test}])";
	}
