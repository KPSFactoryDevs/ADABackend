...
class Segment extends Filter
{
	...
	{
		return "(xfi:segment(.)[" . ( $this->test ? $this->test : "true()" ) . "])";
	}
	...
	{
		return array( ASPECT_COMPLETE_SEGMENT );
	}
	...
	public function validate( $variableSet, $nsMgr )
	{
		...
		{
			\XBRL_Log::getInstance()->formula_validation( "Segment filter", "Failed to compile test expression",
				array(
					'test' => $this->test,
