...
use lyquidity\xml\MS\XmlNamespaceManager;
  ...
  * CAN BE OPTIMIZED
  * (see ModelFormulaObject.py ModelPrecisionFilter class)
  */
class Precision extends Filter
{
	/**
	 * An expression to return the precision to filter by
	 * @var string
	 */
	public $minimum = null;
	...
	public function process( $localName, $taxonomy, $roleUri, $linkbaseHref, $label, $node, $domNode, $log )
	{
		...
		if ( ! property_exists( $attributes, "minimum" ) )
		{
			$log->formula_validation( "Filters", "Missing value filter 'minimum' attribute", array(
				'error' => 'xbrlve:missingRequiredAttribute'
			) );
		}
		else
		{
			$this->minimum = trim( $attributes->minimum );
	...
	{
		return	"(if (fn:string({$this->minimum}) eq 'INF') then fn:false() " .
            	...
            	"else (xfi:precision(.) ge xs:nonNegativeInteger({$this->minimum}+0.0)))";
	}
