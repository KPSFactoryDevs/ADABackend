 ...
 * Returns true if for every item in the left sequence's set there is a v-equal item in the right sequence's
 * set, and the sets have the same count of members. (Because v-equal is defined by XBRL only for items,
 * operation is restricted to nodes that are items.)
 *
 * @param XPath2Context $context
 * @param NodeProvider $provider
 * @param array $args
 * @return FalseValue|TrueValue	Sets are formed from the items in each sequence, removing duplicates in the
 * 								process of forming sets. Returns true if for every item in the left sequence's
 * 								set there is a v-equal item in the right sequence's set, and the sets have the
 ...
 *
 */
function getVEqualSet( $context, $provider, $args )
{
	try
	{
			...
			foreach ( $doni2 as $item2 )
			{
				...
				if ( in_array( $index, $matches ) )
				{
					continue;
 					...
 					if ( getVEqual( $context, $provider, array( XPath2NodeIterator::Create( $item1->CloneInstance() ), XPath2NodeIterator::Create( $item2->CloneInstance() ) ) ) instanceof CoreFuncs::$True )
				{
					$matches[] = $index;
					$match = true;
					break;
