...
/**
 * Returns a QName of the link parent element of an effective relationship's arc. May be helpful to designate base set when
 * multiple link elements can be used with an arcrole.
 *
 * @param XPath2Context $context
 * @param NodeProvider	$provider
 * @param array $args
 * @return xs:QName	Returns the typed QName value of the relationship's parent link element.
 *
 ...
 */
function relationshipLinkName( $context, $provider, $args )
{
	try
	{
		...
		foreach ( $roleTypes as $roleTypeId => $roleType )
		{
			if ( isset( $roleType[ $relationship->query['linkRole'] ] ) )
			{
				$result = QNameValue::fromNCName( $roleTypeId, $context->NamespaceManager );
				return $result;
			}
		}
