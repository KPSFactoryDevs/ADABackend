...
/**
 * Check the element is a tuple. An exception will be thrown if not
 ...
 * @throws XPath2Exception If the element is not an xbrli:tuple
 */
function checkIsTuple( $context, $element, $raiseException = true )
{
	...
	if ( ! isset( $element['substitutionGroup'] ) || ! $types->resolveToSubstitutionGroup( $element['substitutionGroup'], array( \XBRL_Constants::$xbrliTuple ) ) )
	{
		if ( $raiseException )
		{
			throw XPath2Exception::withErrorCode( "xfie:NodeIsNotXbrlTuple", "The concept is not a tuple" );
		}
		return false;
	}
	return true;
