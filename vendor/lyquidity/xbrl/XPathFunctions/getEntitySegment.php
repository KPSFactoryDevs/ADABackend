 ...
 * @param array $args
 * @return XPath2NodeIterator	Return the segment content of the given entity if present and an empty
 * 								sequence otherwise. The elements and their descendant nodes have the
 * 								appropriate type based on the Post Schema Validation Infoset.
 *
 * This function has one real parameter:
 *
 * item	schema-element(xbrli:item)	The item that the segment element is required for.
 */
function getEntitySegment( $context, $provider, $args )
{
	if ( count( $args ) != 1 )
	{
