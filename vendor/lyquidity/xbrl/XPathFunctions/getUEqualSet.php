 ...
 * 								set there is a u-equal item in the right sequence's set, and the sets have the
 ...
 *
 */
function getUEqualSet( $context, $provider, $args )
{
	try
	{
		...
		foreach ( $doni1 as $item1 )
		{
			...
			foreach ( $doni2 as $item2 )
			{
				...
				if ( getUEqual( $context, $provider, array( XPath2NodeIterator::Create( $item1->CloneInstance() ), XPath2NodeIterator::Create( $item2->CloneInstance() ) ) ) instanceof CoreFuncs::$True )
				{
					$matches[] = $index;
					$match = true;
					break;
