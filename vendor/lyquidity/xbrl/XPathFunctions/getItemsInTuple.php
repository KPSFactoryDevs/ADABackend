...
/**
 * Returns the sequence of all fact items that are direct children of the tuple element of an XBRL instance document
 * (excluding any items further nested in enclosed tuples).
 *
 * @param XPath2Context $context
 * @param NodeProvider $provider
 * @param array $args
 * @return element(xbrli:item)*	Returns a sequence of direct child item elements of the tuple element in the XBRL instance,
 * 								or the empty sequence if none. The data type of each item has the appropriate type based on
 ...
 *
 */
function getItemsInTuple( $context, $provider, $args )
{
	try
	{
		...
		$result = InstanceFactsIterator::fromIterator( $context, $args[0], XFI_TUPLE_ITEMS);
