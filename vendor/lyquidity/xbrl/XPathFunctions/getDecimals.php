 ...
 */
function getDecimals( $context, $provider, $args )
{
	if ( count( $args ) != 1 )
	{
	...
	try
	{
		...
		if ( $nav->getLocalName() == "decimals" )
		{
			$result = $nav->getValue();
			return $result == "INF" || $result == "-INF"
				? $result
				: Integer::FromValue( $result );
		}

		$result = \XBRL_Instance::inferDecimals( $value, $nav->getValue() );
