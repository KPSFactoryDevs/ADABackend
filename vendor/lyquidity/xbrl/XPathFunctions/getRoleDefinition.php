 ...
 *
 * role		xs:string?	The subject arcrole.
 *
 */
function getRoleDefinition( $context, $provider, $args )
{
	try
	{
		...
		$types = $taxonomy->getRoleTypes();
		foreach ( $taxonomy->getRoleTypes() as $linkRole => $roles )
		{
			if ( ! isset( $roles[ (string)$args[0] ]['definition'] ) )
			{
				continue;
			}

			$definition = empty( $roles[ (string)$args[0] ]['definition'] )
				? EmptyIterator::$Shared
				: $roles[ (string)$args[0] ]['definition'];
