 ...
 * @param array $args
 * @return xs:boolean	Returns true if the given dimension is a typed dimension and the fact reports a value for
 * 						it in either segment, scenario. This function does not raise an exception if the dimension
 * 						does not apply to the primary item of the fact, but instead returns false if so.
 *
 ...
 *
 */
function getFactHasTypedDimension( $context, $provider, $args )
{
	try
	{
		...
		{
			$dimension = $dimTaxonomy->context->dimensionDefaults[ $key ];
			if ( ! isset( $dimension['typedDomainRef'] ) )
			{
				return CoreFuncs::$False;
			}
		}
		...
		while ( $segment->MoveNext( XPathNodeType::Element ) )
		{
			...
			do
			{
				// Check to see if this is an typed dimension
				if ( $members->getLocalName() == "typedMember" &&
