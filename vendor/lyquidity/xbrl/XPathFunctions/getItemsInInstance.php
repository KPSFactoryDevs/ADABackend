...
/**
 * Returns the sequence of all fact items that are direct children of the root element of an XBRL instance document
 * (excluding any items nested in tuples). The data type of each fact item has the appropriate type based on the
 * Post Schema Validation Infoset.
 *
 * @param XPath2Context $context
 * @param NodeProvider $provider
 * @param array $args
 * @return element(xbrli:item)*	Returns a sequence of root level fact item elements (that are not nested in tuples)
 * 								in the XBRL instance, or the empty sequence if none.
 *
 ...
 *
 */
function getItemsInInstance( $context, $provider, $args )
{
	try
	{
		...
		$result = InstanceFactsIterator::fromIterator( $context, $args[0], XFI_INSTANCE_ITEMS );
