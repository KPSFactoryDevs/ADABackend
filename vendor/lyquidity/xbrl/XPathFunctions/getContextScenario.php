 ...
 * @param array $args
 * @return XPath2NodeIterator	Return the scenario content of the given context if present and an
 * 								empty sequence otherwise. The elements and their descendant nodes
 * 								have the appropriate type based on the Post Schema Validation Infoset.
 *
 * This function has one real parameter:
 *
 * item	schema-element(xbrli:item)	The item that the scenario element is required for.
 */
function getContextScenario( $context, $provider, $args )
{
	if ( count( $args ) != 1 )
	{
