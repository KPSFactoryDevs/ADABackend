 ...
 * Returns true if for every item in the left sequence's set there is a c-equal item in the right sequence's
 * set, and the sets have the same count of members. (Because c-equal is defined by XBRL only for items,
 ...
 * 								set there is a c-equal item in the right sequence's set, and the sets have the
 ...
 *
 */
function getCEqualSet( $context, $provider, $args )
{
	try
	{
		...
		foreach ( $doni1 as $item1 )
		{
			...
			foreach ( $doni2 as $item2 )
			{
				...
				if ( getCEqual( $context, $provider, array( XPath2NodeIterator::Create( $item1->CloneInstance() ), XPath2NodeIterator::Create( $item2->CloneInstance() ) ) ) instanceof CoreFuncs::$True )
				{
					$matches[] = $index;
					$match = true;
					break;
