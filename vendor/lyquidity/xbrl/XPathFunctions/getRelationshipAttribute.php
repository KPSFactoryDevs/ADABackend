 ...
 * @param array $args
 * @return xs:AnyType?	Returns the typed atomic value of the specified attribute of the relationship's arc,
 * 						if it exists and has a schema definition, xs:untypedAtomic value if no schema definition,
 * 						or the empty sequence if attribute absent.
 *
 ...
 */
function getRelationshipAttribute( $context, $provider, $args )
{
	try
	{
		...
		if ( ! property_exists( $relationship, "attributes" ) ||
			 ! isset( $relationship->attributes[ $attribute->LocalName ] )
		)
		{
			return EmptyIterator::$Shared;
		}

		$schemaType = DOMSchemaType::fromSchemaType( qname( $relationship->attributes[ $attribute->LocalName ]['type'], $context->NamespaceManager->getNamespaces() ) );

		$sequenceType = SequenceType::WithTypeCode( $schemaType->TypeCode );
		$value = XPath2Convert::ChangeType( XmlSchema::$String, $relationship->attributes[ $attribute->LocalName ]['value'], $sequenceType, $context->NameTable, $context->NamespaceManager );
		return $value;

		$result = XPath2Item::fromValueAndType( $relationship->attributes[ $attribute->LocalName ]['value'], $schemaType );
		return  $result;
	}
	catch ( \Exception $ex)
	{
