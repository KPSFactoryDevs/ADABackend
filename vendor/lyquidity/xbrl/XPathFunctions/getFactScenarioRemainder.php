...
/**
 * Return the content of a scenario that is not reporting a XBRL Dimensions Specification based dimension value.
 *
 * @param XPath2Context $context
 * @param NodeProvider	$provider
 * @param array $args
 * @return XPath2NodeIterator	Returns the sequence of elements in the scenario for the specified item that do not report
 * 								values for XBRL dimensions. The sequence of elements has the same order as the document
 * 								order of the elements in the item's scenario. The elements and their descendant nodes have
 ...
 *
 */
function getFactScenarioRemainder( $context, $provider, $args )
{
	try
	{
		...
		$scenarioIterator = getScenario( $context, $provider, $args );

		if ( $scenarioIterator->getCount() != 1 )
		{
			return EmptyIterator::$Shared;
		}

		if ( ! $scenarioIterator->MoveNext() )
		{
			return null;
		}

		$current = $scenarioIterator->getCurrent()->CloneInstance();
