...
if headings.n > 0:
    ...
    headings[0].to_json('/var/www/html/staging/jsons/head.json')
    result = []
    with open('/var/www/html/staging/jsons/head.json') as json_file:
        try:
            result.append(json.load(json_file))
        except ValueError:
            print(f)

    with open("/var/www/html/staging/merged_file.json", "w") as outfile:
